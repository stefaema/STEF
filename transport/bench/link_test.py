...
from shared.bench_api import Level, Outcome, Result, Status
...
STATUS = {
    fw_probe.Finding.RUNNING: Status.PASSED,
    fw_probe.Finding.STALE: Status.WARNED,
    fw_probe.Finding.PROTOCOL: Status.FAILED,
    fw_probe.Finding.ABSENT: Status.FAILED,
}

LEVEL = {
    Status.PASSED: Level.OK,
    Status.WARNED: Level.WARN,
    Status.FAILED: Level.ERROR,
    Status.SKIPPED: Level.OK,
}


def _result(status: Status, verdict: fw_probe.Verdict) -> Result:
...
def _descriptor_says(seen: fw_probe.Candidate) -> tuple[Status, str, str]:
...
class VerifyPort:
    ...
    @bench_api.step
    def bootloader(self, bench: object) -> Outcome:
        """ROM bootloader."""
        try:
