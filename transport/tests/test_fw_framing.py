...
def test_a_frame_is_what_lies_between_two_delimiters():
    splitter = fw.framing.FrameSplitter()
...
def test_a_run_past_the_bound_is_dropped_and_the_next_one_survives():
    splitter = fw.framing.FrameSplitter()
    assert splitter.feed(b"x" * (fw.framing.MAX_ENCODED + 1)) == []
...
def test_a_request_survives_the_round_trip_through_the_wire_format():
    ...
    stream = fw.framing.encode(
        fw.framing.seal_request(
            0x1234, fw_api.RPC_NS_RAW, fw_api.RPC_RAW_READ, bytes(args)
        )
    )

    runs = fw.framing.FrameSplitter().feed(stream)
    assert len(runs) == 1

    frame = fw.framing.open_frame(runs[0])
...
def test_the_encoding_carries_no_delimiter_of_its_own():
    ...
    frame = fw.framing.seal_request(0, fw_api.RPC_NS_RAW, fw_api.RPC_RAW_READ, payload)
    encoded = fw.framing.encode(frame)
...
def test_a_corrupted_frame_does_not_open():
    frame = bytearray(
        fw.framing.seal_request(7, fw_api.RPC_NS_SYS, fw_api.RPC_SYS_STATE, b"")
    )
    frame[fw_api.RPC_HDR_LEN - 1] ^= 0xFF
    runs = fw.framing.FrameSplitter().feed(fw.framing.encode(bytes(frame)))
    assert fw.framing.open_frame(runs[0]) is None


def test_a_run_that_is_not_cobs_does_not_open():
    assert fw.framing.open_frame(b"\x05ab") is None


def test_a_payload_too_large_to_frame_is_refused():
    with pytest.raises(fw.framing.FramingError):
        fw.framing.seal_request(
