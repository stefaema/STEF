...
def test_packing_a_batch_of_structs_counts_the_elements_not_the_bytes():
    ...
    packed = fw_api.pack(fw_abi.rpc_raw_write_args, {"idx": 0, "ops": ops})
...
def test_unpacking_recovers_the_elements_the_count_promises():
    ...
    out = fw_api.unpack(fw_abi.rpc_sys_devices_ret, bytes(head) + bytes(devs))
    assert out.count == 2
...
def test_a_reply_truncated_at_its_tail_still_unpacks():
    ...
    out = fw_api.unpack(fw_abi.rpc_relay_send_ret, short)
...
def test_a_count_the_payload_does_not_back_is_refused():
    ...
    with pytest.raises(fw_api.ApiError):
        fw_api.unpack(fw_abi.rpc_sys_devices_ret, bytes(head))
...
def test_the_surface_binds_every_namespace_to_one_carrier():
    seen = []
    surface = fw_api.surface(lambda spec, args, kwargs: seen.append(spec.name))
