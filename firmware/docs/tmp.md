...
**Por el cable viajan structs, con el layout declarado campo por campo.** Cada campo cae en un offset múltiplo de su propio ancho, por medio de miembros `_pad` que están escritos en el struct. Los escalares son de ancho fijo: un estado, un booleano y un enum viajan todos como `uint8_t`. Y cada struct afirma su propio tamaño con un `_Static_assert`, así que un layout que se corre deja de compilar en la punta que se corrió, que es donde alguien puede hacer algo al respecto.

**Los tres encabezados miden ocho bytes.** En xtensa una lectura de 32 bits desalineada es una excepción, no una lectura lenta, así que dónde arranca el payload es una cuestión de corrección. A ocho bytes de encabezado, el payload empieza siempre en un offset múltiplo de cuatro y sus campos quedan alineados solos. Eso es lo que habilita el patrón que usan los handlers: se le pasa a la librería un puntero adentro de la trama de respuesta, y el valor que devuelve no existe en ningún otro lado.

**El layout se escribe una sola vez.** Escribirlo de los dos lados son dos implementaciones de un mismo acuerdo, y la segunda se desincroniza. Así que `rpc_api.h` no incluye más que `stdint.h` y `rpc_proto.h`: la PC lo parsea con cffi y obtiene los mismos structs que compiló el firmware. Ese es también el motivo de que no haya CBOR ni protobuf: existen para negociar estructura entre partes que no pueden compartir código, y estas dos sí pueden.

**El protocolo se lee en un archivo.** Los argumentos y los valores de retorno de cada método son un struct en `rpc_api.h`, al lado del número de método que los nombra. Qué toma `raw.move` es una declaración que se lee, y las dos puntas compilan esa.
...
Dos cosas que el diagrama deja ver mejor que la prosa. El encabezado se sella al final, cuando ya se sabe qué estado lleva la respuesta y cuánto mide, así que se escribe entero y de una sola vez. Y la longitud que cada método espera vive en la tabla, al lado de la función: `rpc_dispatch` la chequea antes de llamar, y el handler recibe un payload que ya mide lo que debía.

Los métodos que llevan un lote, una tira de bytes o una lista son la excepción, porque la cuenta que zanja su longitud viaja adentro del payload, donde `rpc_dispatch` no puede verla. Ahí la tabla aporta un mínimo y el handler concilia el resto en un `if`. Son cinco de veintiséis.
